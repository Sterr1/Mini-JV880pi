...
//
#ifndef _minijv880_robustky040_h
#define _minijv880_robustky040_h
...
class CRobustKY040	/// Driver for KY-040 rotary encoder module
{
public:
	enum TEvent
	{
...
public:
	...
	CRobustKY040 (unsigned nCLKPin, unsigned nDTPin, unsigned nSWPin, CGPIOManager *pGPIOManager = 0);
...
private:
	...
	// Robust Gray-code decoder state.
	// bit1 = CLK, bit0 = DT.
	// The accumulator counts quarter-step deltas and emits one event
	// when the encoder returns to idle 11 with threshold +/-3.
	unsigned m_nEncoderLastCode;
	int m_nEncoderAccumulator;
